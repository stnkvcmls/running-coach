...
def _set_sync_status(db: Session, key: str, value: str):
    ...
    if row:
        ...
        row = SyncStatus(key=key, value=value, updated_at=datetime.utcnow())
        db.add(row)
    db.commit()
...
def sync_activities() -> list[Activity]:
    ...
    try:
        ...
        _set_sync_status(db, "last_activity_sync", datetime.utcnow().isoformat())
...
def sync_daily_summary(target_date: date | None = None) -> DailySummary | None:
    ...
    try:
        ...
        if existing:
            existing.steps = stats.get("totalSteps")
            existing.total_calories = stats.get("totalKilocalories")
            existing.active_calories = stats.get("activeKilocalories")
            existing.resting_hr = stats.get("restingHeartRate")
            existing.max_hr = stats.get("maxHeartRate")
            existing.stress_avg = stress_avg
            existing.sleep_seconds = sleep_seconds
            existing.sleep_score = sleep_score
            existing.body_battery_high = stats.get("bodyBatteryHighestValue")
            existing.body_battery_low = stats.get("bodyBatteryLowestValue")
            existing.intensity_minutes = (
                (stats.get("moderateIntensityMinutes") or 0)
                + (stats.get("vigorousIntensityMinutes") or 0)
            )
            existing.floors_climbed = stats.get("floorsAscended")
            existing.raw_json = json.dumps(raw, default=str)
            existing.synced_at = datetime.utcnow()
            existing.ai_analyzed = False
            db.commit()
            db.refresh(existing)
            summary = existing
        else:
        ...
        _set_sync_status(db, "last_daily_sync", datetime.utcnow().isoformat())
